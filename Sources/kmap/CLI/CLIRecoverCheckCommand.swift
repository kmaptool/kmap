    ...
    static func recoverCheck(_ arguments: [String]) async -> Int32 {
        ...
        guard flags.positionals.count == 2 else {
            return CLIOutput.failure(
                "usage: kmap recover-check <original.img>"
                    + " <rebuilt.img> [--extract=FILE.pbf]…",
                code: 2
            )
        }
        ...
        do {
            ...
            let original = try await StyleRecovery.run(
                img: Paths.expand(flags.positionals[0]),
                extracts: extracts,
                log: log,
                rulesDirectory: neutral
            )
            let rebuilt = try await StyleRecovery.run(
                img: Paths.expand(flags.positionals[1]),
                extracts: extracts,
                log: log,
                rulesDirectory: neutral
            )
            ...
            for (tag, theirCodes) in original.codesByTag.sorted(by: { $0.key < $1.key }) {
                ...
                guard !ourCodes.isEmpty else {
                    ...
                    if rebuilt.groundTags[tag] ?? 0 >= fewestCompared {
                        missing.append(
                            (
                                tag, theirTop.map(\.0).joined(separator: ","),
                                witnesses
                            )
                        )
                    }
            ...
            CLILog.line("")
            CLILog.line(
                "tags compared: \(same + different.count),"
            ...
            for m in missing {
                CLILog.line(
                    String(
                        format: "  MISS  %-40@ theirs %@ ×%d, ours nothing",
                        m.tag as NSString,
                        m.theirs,
                        m.count
                    )
