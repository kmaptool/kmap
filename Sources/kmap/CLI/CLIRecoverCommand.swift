...
extension CLI {
    ...
    static func neutralRules(log: Log) async throws -> URL {
        ...
        return try await catalog.neutralRulesForRecovery(
            log: log,
    ...
    static func recover(_ arguments: [String]) async -> Int32 {
        ...
        do {
            ...
            if let refusal = writeRecoveredStyle(
                report,
                mapPath: path,
                to: flags.value("out"),
    ...
    private static func writeRecoveredStyle(
        ...
        if !report.contested.isEmpty {
            ...
            for port in report.contested {
                ...
                CLILog.line(
                    "  \(kind) 0x\(String(port.ours, radix: 16))"
        ...
        for entry in report.uncovered.prefix(40) {
            let kind = entry.kind.rawValue.padding(
                toLength: 8,
                withPad: " ",
                startingAt: 0
            )
            CLILog.line(
                "  \(kind) 0x\(String(entry.theirs, radix: 16))"
    ...
    private static func printRecovery(
        _ report: StyleRecovery.Report,
        ...
        for o in ordered {
            ...
            CLILog.line(
                "\(code)  \(o.status.rawValue.padding(toLength: 14, withPad: " ", startingAt: 0))"
    ...
    private static func reportRecoveryJSON(
        _ report: StyleRecovery.Report,
        ordered: [StyleRecovery.Outcome],
        path: String,
        out: String?
        ...
        CLIOutput.result([
            ...
            "frame": [
                "minLat": .double(report.frame.minLat),
    ...
    private static func updateLibraryStyle(
        _ report: StyleRecovery.Report,
        mapPath path: String
    ) -> Int32? {
        ...
        guard
            let name = held.exact[
                TypLibrary.fingerprint(
                    ofTypAt: Paths.expand(path)
                )
            ]
        else {
            return CLIOutput.failure(
                "--attach: this map's TYP is not in the library"
    ...
    private static func suggestExtracts(for frame: BBox, path: String) async -> Int32 {
        ...
        CLIOutput.result([
            ...
            "suggested": .array(
                wanted.prefix(6).map { candidate in
                    [
