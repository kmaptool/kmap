...
final class ReassignScreen: Screen {
    ...
    private func objection(to rule: TypeMeaning.Rule) -> String? {
        ...
        case 0:
            return t(
                "this rule is not in %@ as written — it may come from an "
                    + "include, which cannot be substituted",
                kind.ruleFile
            )
        case 1:
            return nil
        default:
            return t(
                "this exact rule appears more than once in %@; moving it "
                    + "would move every copy",
                kind.ruleFile
            )
        }
    }
    ...
    private func handleTarget(_ key: KeyEvent) -> Route {
        ...
        case .backspace:
            if typing {
                ...
                query.removeLast(); list.selected = 0
            }
        case .char(let c):
    ...
    private func renderSources(_ s: Surface, rect: Rect, theme: Theme) {
        ...
        for chunk in wrapText(
            t(
                "%@ is free. Pick the feature to bind to it — its rule moves here and "
        ...
        for i in 0..<min(max(1, rect.maxY - y - 1), shown.count - list.offset) {
            ...
            Widgets.row(
                s,
                rect: Rect(x: rect.x, y: y + i, w: rect.w, h: 1),
                y: y + i,
    ...
    private func renderTargets(_ s: Surface, rect: Rect, theme: Theme) {
        ...
        for i in 0..<min(listHeight, shown.count - list.offset) {
            ...
            var x = s.text(
                rect.x,
                y,
                index == list.selected ? "\(Glyph.arrowRight) " : "  ",
            ...
            x = s.text(
                x,
                y,
                truncate(row.name(preferringRussian: true), to: 26)
            ...
            s.text(
                x,
                y,
                truncate(row.tags.joined(separator: ", "), to: room),
    ...
    private func warnings(
        ...
        if kind == .point, !ReassignScreen.poiCardRange.contains(target >> 8), y < rect.maxY {
            s.text(
                ...
                y,
                t(
                    "outside the POI range 0x2900–0x30ff — the device will draw it "
