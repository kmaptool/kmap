...
extension RecipeForm {
    ...
    func choice(for field: Field, _ ctx: AppContext) -> Choice? {
        ...
        case .interval:
            return among(
                intervals,
                current: recipe.contourInterval,
                label: { "\($0) m" },
                listable: recipe.contours
        ...
        case .demSource:
            return among(
                ...
                label: {
                    $0 == BuildRecipe.recommendedDEMSources
                        ? $0 + "  " + t("(recommended)") : $0
                },
        ...
        case .codePage:
            return among(
                codePageChoices,
                current: recipe.codePage,
                label: RecipeForm.codePageLabel
            ) { self.recipe.codePage = $0 }
        case .descriptions:
            return among(
                BuildRecipe.DescriptionCarrier.allCases,
                current: recipe.descriptions,
                label: \.label
            ) { self.recipe.descriptions = $0 }
        case .theme:
            return among(
                TypEdit.Theme.allCases,
