...
final class MainMenuScreen: Screen {
    ...
    func render(into s: Surface, rect: Rect, ctx: AppContext) {
        ...
        let right = Rect(
            x: rect.x + leftWidth + 3,
            y: bodyY,
            w: max(0, rect.w - leftWidth - 3),
            h: rect.h - 3
        )
    ...
    private func renderStatus(_ s: Surface, rect: Rect, ctx: AppContext) {
        ...
        func line(_ label: String, _ value: String, tone: Color? = nil) {
            ...
            s.text(
                rect.x + 14,
                y,
                truncate(value, to: max(0, rect.w - 14)),
        ...
        case .idle, .loading:
            line(
                t("regions"),
                t("loading %@", String(Widgets.spinner(ctx.frame))),
                tone: theme.dim
            )
        case .ready:
    ...
    static func about(_ key: String) -> String {
        ...
        case "7":
            return t(
                "Folders, memory, download connections, and the language of these"
                    + " screens."
            )
        default:
            return t(
                "How a build works step by step, what the settings mean, and what"
