...
extension StyleCatalog {
    ...
    static let barrierBlockNote =
        "# kmap: one rule per context, so each can be hidden on"
    ...
    func addAerialwayRules(in directory: URL, cyrillic: Bool, log: Log) throws {
        guard
            try spliceRules(
                Self.aerialwayLineRules(cyrillic: cyrillic),
