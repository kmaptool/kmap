...
extension StyleRecovery {
    ...
    static func readCodes(
        ...
        for (key, code) in evidence.codes.sorted(by: { $0.key < $1.key }) {
            var outcome = Outcome(
                kind: code.kind,
                type: code.type,
                witnesses: code.sources.count,
                elements: code.elements,
                unmatched: code.unmatched,
        ...
        -> Outcome
    {
        ...
        let mine = chosen.filter { pair in
            if Double(pair.value.count)
                >= stray * Double(leader[pair.key] ?? pair.value.count)
            {
                return true
            }
            ...
            if Double(pair.value.count)
                >= stray * Double(classLeader[classed] ?? pair.value.count)
            {
                return true
            }
            ...
            if let finest = entry.code.resolutions.keys.max(),
                ...
                Double(pair.value.count) >= stray * Double(entry.meant)
            {
                return true
            }
        ...
        for (bucket, meaning) in mine {
            ...
            guard !meaning.lines.isEmpty, !(openOnly && reAims) else {
                        ...
                        ] ?? witnesses)
                {
                    additions[code.kind, default: []].append(
                        RuleAddition(
                            tag: tag,
                            key: String(tag.split(separator: "=")[0]),
                            type: code.type,
                            witnesses: witnesses,
                            ids: meaning.ids,
            ...
            if meaning.isBuilt,
                meaning.tags.keys.contains(where: {
            ...
            for line in meaning.lines where !line.emits(code.type) {
                ...
                claim.codes.append(
                    (
                        code.type, meaning.count, meaning.ids, meaning.tags,
                        meaning.resolutions
                    )
                )
                claimed[slot] = claim
    ...
    private static func chooseMeanings(
        of entry: CodeReading,
        ...
        -> [(key: String, value: MeaningBucket)]?
    {
        ...
        guard entry.meant >= scarce, let family,
            Double(held) / Double(plausible) >= purity
        else {
            if let led = mopUp(
                of: entry,
                ranked: ranked,
