    ...
    func applyZoomPlan(
        ...
        for file in Set(asked.flatMap(\.files)).sorted() {
            ...
            for rule in ZoomRuleScan.rules(in: lines) {
                ...
                guard
                    ...
                    let was = rungs.rung(forResolution: found.value)
                else { continue }
            ...
            lines.insert(
                "# kmap: resolutions set by the zoom plan \"\(plan.name)\""
        ...
        for family in asked {
            ...
            if n == 0 {
                ...
                log.append(
                    "zoom plan: \(family.nameKey) — \(n) rule(s) on rung(s)"
