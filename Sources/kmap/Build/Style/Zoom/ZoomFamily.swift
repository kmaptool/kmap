...
struct ZoomFamily: Identifiable, Equatable {
    ...
    static let all: [ZoomFamily] = [
        ZoomFamily(
            id: "contours",
            nameKey: "Contour lines",
            noteKey: "the elevation lines and their labels",
            files: ["inc/contour_lines"],
            tags: [],
            wholeFile: true
        ),
        ...
        ZoomFamily(
            id: "trails",
            nameKey: "Trails and tracks",
            noteKey: "paths, tracks, footways, steps, via ferrata — what you walk",
            files: ["lines"],
            tags: [
                "highway=path", "highway=track", "highway=footway",
        ...
        ZoomFamily(
            id: "railways",
            nameKey: "Rail and cableways",
            noteKey: "rail, tram, funicular and the aerialways",
            files: ["lines", "polygons"],
            tags: ["railway=*", "aerialway=*"]
        ),
        ZoomFamily(
            id: "water",
            nameKey: "Water",
        ...
        ZoomFamily(
            ...
            files: ["lines", "polygons"],
            tags: [
                "landuse=*", "amenity=*", "man_made=*", "shop=*", "tourism=*",
    ...
    private static func found(
        ...
        while let hit = condition.range(of: needle, range: from..<condition.endIndex) {
            let before: Character? =
                hit.lowerBound == condition.startIndex
                ? nil : condition[condition.index(before: hit.lowerBound)]
            let after: Character? =
                hit.upperBound == condition.endIndex
