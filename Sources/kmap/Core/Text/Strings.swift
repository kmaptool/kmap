...
enum Strings {
    ...
    private static let russian: [String: String] = [
        ...
        "The file is rewritten at once. A new section can be created with a, but the current colours and drawing will be lost.": "Файл перезаписывается сразу. Новую секцию можно будет создать клавишей a, но текущие цвета и рисунок будут потеряны.",
        "The section for %@ will be deleted: its drawing, its colours and its names. The device will then draw this type its own way.": "Секция для %@ будет удалена: рисунок, цвета и названия. Устройство будет рисовать этот тип по-своему.",
        "delete the section": "удалить секцию",
        "drawn for": "рисуется для",
        "name": "название",
        "section for %@ deleted — the device will draw this type its own way": "секция для %@ удалена — устройство будет рисовать этот тип по-своему",
        "type": "тип",
