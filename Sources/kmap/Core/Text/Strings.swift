...
enum Strings {
    ...
    private static let russian: [String: String] = [
        ...
        "administrative boundaries (~2.5 GB — this one takes a while)": "административные границы (~2,5 ГБ — это надолго)",
        ...
        "descriptions": "описания",
        "output folder": "путь",
        "In OSM two roads can touch on screen yet share no point — for the router that is a dead end, and no route crosses it. This joins ends closer than %d m, and only where there is no way through at all: an existing route is never changed, however long the detour.": "В OSM две дороги могут сходиться на экране, но не иметь общей точки — для роутера это тупик, и маршрут через него не пройдёт. Сшивка соединяет концы ближе %d м, и только там, где проезда нет вовсе: существующий маршрут не меняется, каким бы длинным ни был объезд.",
        "Never through a building, a fence, a wall or a hedge — whether a plot can be crossed is OSM's to say — and never a footway onto the road it runs beside.": "Никогда — сквозь здание, забор, стену или живую изгородь (можно ли пройти через участок, решает OSM) и никогда — тротуар с дорогой, вдоль которой он идёт.",
        "A kerb, a bank or a step between the two ends is crossed by a separate short link, drawn as a thin dotted line, so the map shows what it steps over.": "Бордюр, откос или ступенька между концами пересекаются отдельной короткой связкой — тонким пунктиром, чтобы на карте было видно, через что она ведёт.",
        "In OSM two roads can touch on screen yet share no point; for the router that gap is a wall. kmap joins ends a few metres apart, and only where no route gets through at all — an existing way round is never touched.": "В OSM две дороги могут сходиться на экране, но не иметь общей точки; для роутера такой разрыв — стена. kmap соединяет концы в нескольких метрах друг от друга, и только там, где проезда нет вовсе: существующий объезд не трогается.",
        "Never through a building, a fence or a hedge — whether a plot can be crossed is OSM's to say. A kerb or a step between the ends is crossed by a separate link drawn as a thin dotted line, so the map shows what it steps over.": "Никогда — сквозь здание, забор или живую изгородь: можно ли пройти через участок, решает OSM. Бордюр или ступенька между концами пересекаются отдельной связкой — тонким пунктиром, чтобы на карте было видно, через что она ведёт.",
