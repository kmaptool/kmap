...
final class RecipeProfileTests: XCTestCase {
    ...
    override func setUp() {
        ...
        MainActor.assumeIsolated {
            ...
            settings.update {
                $0.profiles = [
                    BuildProfile(id: "coarse", name: "Coarse", choices: coarse),
    ...
    func testTheOverlapRowsFollowTheSeamPatch() async {
        XCTAssertFalse(
            drawn(screen()).contains("Land overlap"),
    ...
    func testThePanelSaysWhatTheElevationWillCost() async {
        ...
        XCTAssertTrue(
            frame.lowercased().contains("elevation"),
            "the block has to be there when a build takes elevation at all"
        )
        XCTAssertTrue(
            frame.contains("costs to fetch") || frame.contains("to download")
                || frame.contains("cell"),
    ...
    func testAChangeOnTheFormIsSaidOutLoudAndNotWrittenDown() async {
        ...
        XCTAssertTrue(
            frame.contains("changed for this map only"),
