...
final class ToolLocationsTests: XCTestCase {
    ...
    func testTheMacShimIsAskedOnAMacAndNowhereElse() {
        var asked = 0
        let home = { () -> String? in
            asked += 1; return "/Library/Java/JavaVirtualMachines/21.jdk/Contents/Home"
        }
        let mac = ToolLocations.java(
            on: .macOS,
            environment: [:],
            which: noProgram,
            contents: nothing,
    ...
    func testJavaHomeOnWindowsIsSpelledTheWindowsWay() {
        let candidates = ToolLocations.java(
            on: .windows,
            environment: ["JAVA_HOME": #"C:\Program Files\Microsoft\jdk-21"#],
            which: noProgram,
            contents: nothing,
            macJavaHome: { nil }
        )
    ...
    func testAPerUserInstallIsLookedForToo() {
        ...
        let candidates = ToolLocations.java(
            ...
            which: noProgram,
            contents: {
                $0 == #"C:\Users\k\AppData\Local\Programs\Eclipse Adoptium"#
    ...
    func testJavacIsFoundBesideJavaAndNotOnThePath() {
        ...
        XCTAssertEqual(
            ToolLocations.companion(
                "javac",
                of: "/usr/lib/jvm/jdk-21/bin/java",
                on: .linux
            ),
    ...
    func testOnWindowsItIsJavacExeAndSayingOtherwiseLosesTheJdk() {
        ...
        let javac = ToolLocations.companion(
            "javac",
    ...
    func testAWindowsPythonIsFoundWhereItsInstallerPutsIt() {
        let candidates = ToolLocations.python(
            ...
            which: noProgram,
            contents: {
                $0 == #"C:\Users\k\AppData\Local\Programs\Python"#
                    ? ["Python39", "Python312"] : []
            }
        )
        XCTAssertEqual(
    ...
    func testAVirtualEnvironmentKeepsItsProgramsWhereThatPythonPutsThem() {
        ...
        XCTAssertEqual(
            ToolLocations.inVirtualEnvironment("pip", of: venv, on: .linux).path,
    ...
    func testTheClasspathIsSeparatedTheWayJavaSeparatesItOnThatMachine() {
        ...
        XCTAssertEqual(
            ToolLocations.classpathSeparator(on: .wsl),
            ":",
