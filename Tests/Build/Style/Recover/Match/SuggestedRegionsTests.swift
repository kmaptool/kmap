...
final class SuggestedRegionsTests: XCTestCase {
    ...
    func testAContinentWideMapIsOfferedCountriesInsideIt() throws {
        ...
        XCTAssertFalse(
            offered.contains { $0.region.hasChildren },
    ...
    func testAFrameSwollenByAFarTileDoesNotOfferTheCountriesInBetween() throws {
        ...
        XCTAssertTrue(
            offered.contains { $0.region.id == "exclave" },
            "the far tile is real ground too"
        )
        XCTAssertFalse(
            offered.contains { $0.region.id == "neighbour" },
    ...
    func testABorderTileDoesNotMakeANeighbourIntoGround() throws {
        ...
        for (b, weight) in [
            ([27.0, 56.0, 36.0, 62.0], 1000.0),  // the map's mass
            ([26.8, 57.0, 27.5, 60.0], 2.0)
        ] {  // a thin border tile
            let box = BBox(minLon: b[0], minLat: b[1], maxLon: b[2], maxLat: b[3])
            frame.extend(lon: box.minLon, lat: box.minLat)
            frame.extend(lon: box.maxLon, lat: box.maxLat)
        ...
        let offered = RegionSuggestion.suggestedRegions(
            on: RegionSuggestion.DrawnGround(spots: spots, frame: frame),
            index: try index()
        )
    ...
    func testTheOfferIsTheLightestDownloadWithEnoughOfTheMapInIt() throws {
        ...
        {
            RegionSuggestion.Candidate(
                region: try region(id),
                inside: inside,
                share: 0.5,
                density: 1,
                drawn: drawnMB * 1_048_576
            )
        }
        ...
        XCTAssertEqual(
            RegionSuggestion.worthDownloading(weighed)?.region.id,
            "coastal",
    ...
    func testTheDownloadLandsWhereABuildLooks() throws {
        ...
        XCTAssertEqual(
            RegionSuggestion.cacheDestination(for: coastal).lastPathComponent,
            "coastal.osm.pbf"
        )
        XCTAssertEqual(
            RegionSuggestion.cacheDestination(for: coastal)
