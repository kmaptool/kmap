...
final class DeriveTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        ...
        """.write(
            to: folder.appendingPathComponent("points"),
            atomically: true,
            encoding: .utf8
        )
    ...
    private func forCode(
        _ kind: ElementDumper.Kind,
        _ type: Int,
        seed: inout Int64,
        _ counted: [(tags: [String: String], count: Int)]
    ) -> Evidence.ForCode {
    ...
    func testAMeaningWithNoRuleAtAllGetsOneWritten() {
        ...
        XCTAssertTrue(
            report.sheet.contains("+ natural=ridge [0x11518 resolution"),
    ...
    func testAFamilyMarkIsReadByItsKey() {
        ...
        XCTAssertTrue(
            report.sheet.contains("+ highway=trunk [0x11f00"),
            "the member it is sure of"
        )
        XCTAssertFalse(
            report.sheet.contains("+ highway=trunk_link"),
    ...
    func testARuleLeftOnANumberTheirMapMeansOtherwiseIsSilenced() {
        ...
        let report = derive(
            ...
            ground: [
                "amenity=fuel": 900, "tourism=hotel": 2400,
                "tourism=information": 100
            ],
            typDefined: [.point: [0x2f01, 0x2c01]]
        )
        XCTAssertTrue(
            report.sheet.contains("silenced")
    ...
    func testAHandfulOutOfThousandsIsNotAMeaning() {
        ...
        XCTAssertTrue(
            report.sheet.contains("barrier=lift_gate"),
            "what the code is really for still earns its rule"
        )
        XCTAssertFalse(
            report.sheet.contains("highway=crossing"),
    ...
    func testAHandfulOutOfAHandfulIsAMeaning() {
        ...
        XCTAssertTrue(
            report.sheet.contains("amenity=photo_booth"),
    ...
    func testAStrayMatchDoesNotClaimAMeaningFromTheCodeThatDrawsIt() {
        ...
        XCTAssertTrue(
            report.sheet.contains("+     [0x10407 resolution 22]"),
            "the railway rule is re-aimed, since that is what it draws"
        )
        XCTAssertFalse(
            report.sheet.contains("+ highway=path [0x10407"),
    ...
    func testAGenericMarkClaimsTheWholeFamilyItDraws() {
        ...
        for member in [
            "+ highway=trunk [0x0f", "+ highway=trunk_link [0x0f",
    ...
    func testASideTagOfSomethingWeDrawGetsNoRuleOfItsOwn() {
        ...
        XCTAssertFalse(
            report.sheet.contains("natural=coastline"),
    ...
    func testAYardIsNotAStrayOfTheBuildingsStandingInIt() {
        ...
        XCTAssertTrue(
            report.sheet.contains("+ power=substation & building!=* [0x25"),
            report.sheet
        )
    }
    ...
    func testOpenGroundDoesNotTakeTheBuildingsWithIt() {
        ...
        XCTAssertTrue(
            report.sheet.contains("+ landuse=military & building!=* [0x36"),
            report.sheet
        )
        XCTAssertTrue(
            report.sheet.contains("+ landuse=military & building!=* [0x04"),
...
extension DeriveTests {
    ...
    func testACollidingNumberReAimsAChainItDoesNotOwn() {
        ...
        XCTAssertTrue(
            report.sheet.contains(
                "+ landuse=military [0x1c resolution 20]"
            ),
            report.sheet
        )
    }
    ...
    func testAChainFollowsWhatTheMapActuallyDrew() {
        ...
        XCTAssertTrue(
            report.sheet.contains(
                "+ landuse=forest & leaf_type=needleleaved [0x50 resolution 22]"
            ),
            report.sheet
        )
    }
    ...
    func testARuleTheirTypWouldRepaintIsSilenced() {
        ...
        XCTAssertTrue(
            report.sheet.contains("- landuse=farmland [0x1c resolution 20]"),
            report.sheet
        )
    ...
    func testAFamilyRuleAnswersForTagsWithNoRuleOfTheirOwn() {
        ...
        XCTAssertTrue(
            report.sheet.contains(
                "+ building=* & building!=no [0x99 resolution 24]"
            ),
            report.sheet
        )
        XCTAssertFalse(
            report.sheet.contains(
                "their style draws building=yes, ours had"
                    + " no rule"
            ),
            report.sheet
        )
...
extension DeriveTests {
    ...
    func testTheRulesOwnCodeIsNotLayeredOverItself() {
        ...
        XCTAssertEqual(
            sheet.components(separatedBy: "+ highway=trunk [0x02").count - 1,
            1,
...
extension DeriveTests {
    ...
    func testALineAdditionOverAnAreaKeepsLooking() {
        ...
        XCTAssertTrue(
            report.sheet.contains("+ building=* [0x12207 resolution 22 continue]"),
            report.sheet
        )
    }
}
