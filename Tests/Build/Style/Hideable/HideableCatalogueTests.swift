...
final class HideableCatalogueTests: XCTestCase {
    ...
    func testTheCatalogueIsWrittenBesideThePointsItWasReadFrom() {
        ...
        XCTAssertEqual(
            HideableCatalogue.destination(besidePoints: points).path,
            "/styles/.base-build-1a2b/hideable.txt"
        )
        XCTAssertNotEqual(
            HideableCatalogue.destination(besidePoints: points),
            HideableCatalogue.url,
            "not the shared path the swap removes"
        )
    }
}
