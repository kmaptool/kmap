...
final class SplitAxisTests: RecipeTestCase {
    ...
    func testARegionIsCutAcrossItsLongerSideOnTheGroundNotInDegrees() {
        ...
        XCTAssertEqual(
            SplitAxis.best(for: BBox(minLon: 0, minLat: 0, maxLon: 10, maxLat: 5)),
            .longitude
        )
        XCTAssertEqual(
            SplitAxis.best(for: BBox(minLon: 0, minLat: 0, maxLon: 5, maxLat: 10)),
            .latitude
        )
        // Equal in degrees at 70 deg N: the box is far taller than it is wide on the ground.
        XCTAssertEqual(
            SplitAxis.best(for: BBox(minLon: 20, minLat: 69, maxLon: 24, maxLat: 73)),
            .latitude
        )
