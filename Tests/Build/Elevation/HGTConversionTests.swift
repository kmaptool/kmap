...
final class HGTConversionTests: XCTestCase {
    ...
    private func source(
        ...
        width: Int,
        height: Int,
        heights: (_ row: Int, _ column: Int) -> Float
    ) throws -> URL {
    ...
    private func tallSource(
        cell: (lat: Int, lon: Int),
        width: Int,
        heights: @escaping (_ row: Int, _ column: Int) -> Float
    ) throws -> URL {
    ...
    func testTheFileIsTheShapeAndByteOrderMkgmapReads() throws {
        ...
        let written = try HGTConversion.write(
            cell: (lat: 44, lon: 33),
            from: mosaic(["N44E033": url]),
            to: out
        )
    ...
    func testHeightsAreRoundedHalvesAwayFromZeroAsGDALDoes() throws {
        ...
        let data = try Data(contentsOf: out)
        XCTAssertEqual(
            (0..<values.count).map { height(data, row: 0, column: $0) },
            [1, 2, -1, -4, 2, 3, -2, -3, -430]
        )
    }
    ...
    func testTheEasternColumnOfAThreeArcSecondCellComesFromTheNeighbour() throws {
        ...
        XCTAssertEqual(
            height1201(data, row: 0, column: 1200),
            9,
    ...
    func testTheSouthernRowOfAThinnedCellComesFromTheDenserBandBelow() throws {
        ...
        XCTAssertEqual(
            height1201(data, row: 1, column: 0),
            1,
