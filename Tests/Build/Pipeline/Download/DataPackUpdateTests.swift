...
final class DataPackUpdateTests: XCTestCase {
    ...
    func testEveryBuildAsksEveryTimeAndNeverAsksNever() {
        let now = Date()
        XCTAssertFalse(
            ToolchainUpdates.everyBuild.stillGood(checked: now, now: now),
            "asked again even a second later"
        )
        XCTAssertTrue(
            ToolchainUpdates.never.stillGood(checked: nil, now: now),
    ...
    func testAnIntervalHoldsUntilItHasPassed() {
        ...
        XCTAssertTrue(
            cadence.stillGood(
                checked: now.addingTimeInterval(-29 * 24 * 3600),
                now: now
            )
        )
        XCTAssertFalse(
            cadence.stillGood(
                checked: now.addingTimeInterval(-31 * 24 * 3600),
                now: now
            )
        )
    ...
    func testTheStageSitsBetweenTheToolCheckAndTheDownload() {
        ...
        XCTAssertEqual(
            order.firstIndex(of: .dataUpdate),
            order.firstIndex(of: .preflight).map { $0 + 1 }
        )
        XCTAssertEqual(
            order.firstIndex(of: .download),
