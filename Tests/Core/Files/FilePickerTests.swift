...
final class FilePickerTests: XCTestCase {
    ...
    func testTheFirstOfTheThreeInstalledWins() {
        ...
        let onlyKdialog = FilePicker.helper(
            platform: .linux,
    ...
    func testAWindowsAnswerIsOpenedAsWrittenWhenThereIsNoBoundaryToCross() {
        let chosen = FilePicker.path(
            from: #"E:\Garmin\style.typ"# + "\r\n",
            kind: .powershell,
            translating: false,
            toLinux: { _ in
    ...
    func testTheMacIsAskedForAFileOfTheTypesWanted() {
        let arguments = FilePicker.arguments(
            for: .osascript,
    ...
    private func windowsScript(
        _ wanted: FilePicker.Wanted,
        prompt: String,
    ...
    func testTheAnswerSurvivesEvenIfSomethingElseSerialisesItselfOntoTheEnd() {
        ...
        XCTAssertEqual(
            FilePicker.path(from: noisy, kind: .powershell, translating: true)?.path,
    ...
    func testStrippingTheXmlLeavesAnOrdinaryAnswerAlone() {
        XCTAssertEqual(
            FilePicker.withoutSerialisedObjects(#"E:\Garmin\x.typ"#),
            #"E:\Garmin\x.typ"#
        )
    ...
    func testAPathComesBackAndAnythingElseDoesNot() {
        XCTAssertEqual(
            FilePicker.path(from: "/Volumes/Disk/style.typ\n")?.path,
    ...
    func testAFolderComesBackWithoutItsTrailingSlash() {
        ...
        XCTAssertEqual(
            FilePicker.path(from: "/Users/someone/Maps/")?.path,
            "/Users/someone/Maps"
        )
    ...
    func testAWindowsPathWithSpacesAndCyrillicSurvives() {
        let url = FilePicker.path(
            from: #"E:\Garmin\Карты Региона\style.typ"#,
            kind: .powershell,
            translating: true
        )
    ...
    func testTheOtherDialectsAreStillReadAsPosixPaths() {
        XCTAssertNil(
            FilePicker.path(from: #"E:\Garmin\x.typ"#, kind: .zenity),
            "a Windows path from zenity would be nonsense"
        )
        XCTAssertEqual(
            FilePicker.path(from: "/home/k/x.typ", kind: .zenity)?.path,
            "/home/k/x.typ"
        )
    }
